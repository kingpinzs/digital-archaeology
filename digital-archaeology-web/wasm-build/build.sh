...
#
# PURPOSE:
#   Compiles the Micro4 assembler from C to WebAssembly using Emscripten.
#   The output can run in a Web Worker to assemble programs in the browser.
#
# PREREQUISITES:
...
#
# OUTPUT:
#   ../public/wasm/micro4-asm.js   - Emscripten glue code (ES6 module)
#   ../public/wasm/micro4-asm.wasm - WebAssembly binary
#
# CONFIGURATION:
...
echo "=========================================="
echo "Micro4 Assembler WASM Build"
echo "=========================================="
echo ""
...
# Report file sizes
JS_SIZE=$(ls -lh "$OUTPUT_DIR/micro4-asm.js" | awk '{print $5}')
WASM_SIZE=$(ls -lh "$OUTPUT_DIR/micro4-asm.wasm" | awk '{print $5}')

echo "Output files:"
echo "  $OUTPUT_DIR/micro4-asm.js   ($JS_SIZE)"
echo "  $OUTPUT_DIR/micro4-asm.wasm ($WASM_SIZE)"
echo ""

# Size sanity check (WASM should be <100KB for this simple assembler)
WASM_BYTES=$(stat -c%s "$OUTPUT_DIR/micro4-asm.wasm" 2>/dev/null || stat -f%z "$OUTPUT_DIR/micro4-asm.wasm")
if [ "$WASM_BYTES" -gt 102400 ]; then
    echo "WARNING: WASM file is larger than expected (>100KB)."
    echo "         This may indicate unnecessary code inclusion."
fi
